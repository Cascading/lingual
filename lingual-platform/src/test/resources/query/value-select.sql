...
values ( SQRT(9)) ;
values ( SQRT(20)) ;
values ( POSITION ('ab' IN 'abcdef')) ;
values ( POSITION ('cd' IN 'abcdef')) ;
values ( POSITION ('xy' IN 'abcdef')) ;
values ( SUBSTRING ('abcdef' FROM 3)) ;
values ( SUBSTRING ('abcdef' FROM 3 FOR 2)) ;
values ( TRIM ('abc ') || TRIM (' defg') );
values ( OVERLAY ('abdef' PLACING 'xy' FROM 2) );